...
// -----------------------------------------------------------------------------
    ...
    protected function decodeXref($startxref, $xref = array())
    {
        ...
        if (preg_match('/trailer[\s]*<<(.*)>>[\s\r\n]+(?:[%].*[\r\n]+)*startxref[\s\r\n]+/isU', $this->pdfdata,
                $matches, PREG_OFFSET_CAPTURE, $xoffset) > 0) {
        ...
        switch ($char) {
            ...
            { // \x3E GREATER-THAN SIGN
                if (isset($data[($offset + 1)]) and ($data[($offset + 1)] == $char)) {
                    ...
                    if (($char == '<') and (preg_match('/^([0-9A-Fa-f ]+)[>]/iU', substr($data, $offset),
                                $matches) == 1)) {
                        $objval = $matches[1];
            ...
            default:
            {
                ...
                switch ($frag) {
                    ...
                    case 'ends':
                        ...
                        break;
                    default:
                        if (preg_match('/^([0-9]+)[\s]+([0-9]+)[\s]+([Robj]{1,3})/i', substr($data, $offset, 33),
                                $matches) == 1) {
                            if ($matches[3] == 'R') {
    ...
    protected function getIndirectObject($obj_ref, $offset = 0, $decoding = true)
    {
        ...
        do {
            if (($i > 0) and (isset($objdata[($i - 1)][0])) and ($objdata[($i - 1)][0] == PDF_TYPE_DICTIONARY) and array_key_exists('/Length',
                    $objdata[($i - 1)][1])) {
    ...
    public function getObjectVal($obj)
    {
        if ($obj[0] == PDF_TYPE_OBJREF) {
            ...
            if (isset($this->objects[$key[0]][$key[1]])) {
                ...
                $this->objects[$key[0]][$key[1]] = $this->getRawObject($streaminfo[1],
    ...
    {
        $this->objoffsets = array();
        if (preg_match_all('/(*ANYCRLF)^[\s]*([0-9]+)[\s]+([0-9]+)[\s]+obj/im', $this->pdfdata, $matches,
    ...
    { // $obj = /Page
        ...
        if ($annots !== false && $annots[0] == PDF_TYPE_OBJREF) {
